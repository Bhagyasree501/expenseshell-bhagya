...
mysql -h 172.31.80.119 -u root -pExpenseApp@1 -e 'show databases;' &>> $FILE_NAME
if [ $? -ne 0 ]
then
    echo -e "$Y looks like there is no passowrd for mysql-server yet. let us create one now $N" | tee -a $FILE_NAME
